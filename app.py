...
for mes in meses:
    ...
    with col1:
        fg = st.number_input("Fact General €", value=int(datos_por_mes.get(mes, {}).get("FG", 0)), step=1, key=mes+"fg")
        lg = st.number_input("Lab General €", value=int(datos_por_mes.get(mes, {}).get("LG", 0)), step=1, key=mes+"lg")
        fpsi = st.number_input("Fact PSI €", value=int(datos_por_mes.get(mes, {}).get("FPSI", 0)), step=1, key=mes+"fpsi")
        lpsi = st.number_input("Lab PSI €", value=int(datos_por_mes.get(mes, {}).get("LPSI", 0)), step=1, key=mes+"lpsi")
    ...
    with col2:
        fpsi_v = st.number_input("Fact PSI V €", value=int(datos_por_mes.get(mes, {}).get("FPSI_V", 0)), step=1, key=mes+"fpsi_v")
        lpsi_v = st.number_input("Lab PSI V €", value=int(datos_por_mes.get(mes, {}).get("LPSI_V", 0)), step=1, key=mes+"lpsi_v")
